...
const Crypto = require('./@trrxitte/traaittcash-crypto')
const Mnemonic = require('./mnemonic')
const Numeral = require('numeral')
const SecureRandomString = require('secure-random-string')
const Transaction = require('./transaction')
const traaittCASHCrypto = new Crypto()
...
class CryptoNote {
   ...
   * @param {Buffer|Object|string} [extra] - extra data to include in the transaction
   * @returns {boolean|CryptoNote.GeneratedTransaction} A newly generated transaction
   */
  createTransaction (newOutputs, ourOutputs, randomOuts, mixin, feeAmount, paymentId, unlockTime, extra) {
    const tx = this.createTransactionStructure(newOutputs, ourOutputs, randomOuts, mixin, feeAmount, paymentId, unlockTime, false, extra)
   ...
   * @returns {boolean|CryptoNote.GeneratedTransaction} A newly generated transaction
   */
  createTransactionAsync (newOutputs, ourOutputs, randomOuts, mixin, feeAmount, paymentId, unlockTime, extra) {
    return this.createTransactionStructure(
      newOutputs, ourOutputs, randomOuts, mixin, feeAmount, paymentId, unlockTime, true, extra
    ).then((tx) => {
      return {
        transaction: tx,
        rawTransaction: tx.blob,
   ...
   */
  createTransactionStructure (newOutputs, ourOutputs, randomOuts, mixin, feeAmount, paymentId, unlockTime, _async, extra) {
    return createTransaction(newOutputs, ourOutputs, randomOuts, mixin, feeAmount, paymentId, unlockTime, _async, this.config.maximumOutputAmount, extra)
  }
  ...
  underivePublicKey (derivation, outputIndex, outputKey) {
    ...
    const [err, key] = traaittCASHCrypto.underivePublicKey(derivation, outputIndex, outputKey)
...
function cnFastHash (input) {
  ...
  const [err, hash] = traaittCASHCrypto.cn_fast_hash(input)
...
function createTransaction (newOutputs, ourOutputs, randomOutputs, mixin, feeAmount, paymentId, unlockTime, _async, maximumOutputAmount, extra) {
...
function derivePublicKey (derivation, outputIndex, publicKey) {
  ...
  const [err, key] = traaittCASHCrypto.derivePublicKey(derivation, outputIndex, publicKey)
...
function deriveSecretKey (derivation, outputIndex, privateKey) {
  ...
  const [err, key] = traaittCASHCrypto.deriveSecretKey(derivation, outputIndex, privateKey)
...
function generateKeyDerivation (transactionPublicKey, privateViewKey) {
  ...
  const [err, derivation] = traaittCASHCrypto.generateKeyDerivation(transactionPublicKey, privateViewKey)
...
function generateKeyImage (publicKey, privateKey) {
  ...
  const [err, keyImage] = traaittCASHCrypto.generateKeyImage(publicKey, privateKey)
...
function checkRingSignatures (transactionPrefixHash, keyImage, inputKeys, signatures) {
  ...
  if (userCryptoFunctions.checkRingSignatures) {
    ...
    return traaittCASHCrypto.checkRingSignatures(transactionPrefixHash, keyImage, inputKeys, signatures)
  }
}
...
function generateRingSignaturesPrimitive (transactionPrefixHash, keyImage, inputKeys, privateKey, realIndex) {
  ...
  if (userCryptoFunctions.generateRingSignatures) {
    ...
    const [err, sigs] = traaittCASHCrypto.generateRingSignatures(transactionPrefixHash, keyImage, inputKeys, privateKey, realIndex)
...
function generateSignature (digestHash, publicKey, privateKey) {
  ...
  if (userCryptoFunctions.generateSignature) {
    ...
    [err, signature] = traaittCASHCrypto.generateSignature(digestHash, publicKey, privateKey)
  }
...
function privateKeyToPublicKey (privateKey) {
  ...
  const [err, key] = traaittCASHCrypto.secretKeyToPublicKey(privateKey)
...
function scReduce32 (hex) {
  const [err, result] = traaittCASHCrypto.scReduce32(hex)
...
function verifySignature (digestHash, publicKey, signature) {
  ...
  return traaittCASHCrypto.checkSignature(digestHash, publicKey, signature)
}
